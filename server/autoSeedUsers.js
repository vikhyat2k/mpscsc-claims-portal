/**
 * server/autoSeedUsers.js
 * ==============================================================================
 * Self-healing automatic seeder for MPSCSC test accounts.
 * Ensures that all 10 registered user accounts always have:
 *   - 1 Employee Profile in Employee Master
 *   - 1 Tour Diary (with journey legs)
 *   - 1 TA/DA Claim (with journeys, hotel, and calculated total amount)
 *   - 1 Transfer Claim (with packing, transport, and family details)
 *   - 1 Medical Claim (with patient, illness, and itemized bills)
 * Runs automatically on server startup if any of the users lack records.
 * ==============================================================================
 */
...
function autoSeedDummyUsers(db) {
    try {
        ...
        for (const item of defaultTestUsers) {
            // 1. User
            let user = db.prepare('SELECT id FROM users WHERE email = ?').get(item.user.email);
            if (!user) {
            ...
            // 2. Employee
            let emp = db.prepare('SELECT id, user_id FROM employees WHERE user_id = ?').get(user.id);
            if (!emp) {
                const existingByName = db.prepare('SELECT id, user_id FROM employees WHERE name = ?').get(item.emp.name);
                if (existingByName) {
                    db.prepare('UPDATE employees SET user_id = ? WHERE id = ?').run(user.id, existingByName.id);
                    emp = { id: existingByName.id, user_id: user.id };
                } else {
                    ...
                    emp = { id: info.lastInsertRowid, user_id: user.id };
                }
            }

            // 3. Tour Diary
            let td = db.prepare('SELECT id, total_amount FROM claims WHERE employee_id = ? AND is_diary = 1').get(emp.id);
            ...
            } else if (!td.total_amount || td.total_amount === 0) {
                db.prepare('UPDATE claims SET total_amount = ? WHERE id = ?').run(totalFare, td.id);
            }

            // 4. TA/DA Claim
            let tada = db.prepare("SELECT id, total_amount FROM claims WHERE employee_id = ? AND is_diary = 0 AND claim_type = 'TA_DA'").get(emp.id);
            if (!tada) {
                const renderedClaimId = `CL-${yearSuffix}-${Math.floor(1000 + Math.random() * 9000)}`;
                const info = insertClaim.run(
            ...
            } else if (!tada.total_amount || tada.total_amount === 0) {
                db.prepare('UPDATE claims SET total_amount = ? WHERE id = ?').run(item.tada.total_amount, tada.id);
            }

            // 5. Transfer Claim
            let transfer = db.prepare("SELECT id, total_amount FROM claims WHERE employee_id = ? AND claim_type = 'TRANSFER'").get(emp.id);
            if (!transfer) {
                const renderedClaimId = `CL-${yearSuffix}-${Math.floor(1000 + Math.random() * 9000)}`;
                insertClaim.run(
            ...
            } else if (!transfer.total_amount || transfer.total_amount === 0) {
                db.prepare('UPDATE claims SET total_amount = ? WHERE id = ?').run(item.transfer.total_amount, transfer.id);
            }

            // 6. Medical Claim
            let med = db.prepare("SELECT id, total_amount FROM claims WHERE employee_id = ? AND claim_type = 'MEDICAL'").get(emp.id);
            if (!med) {
                const renderedClaimId = `CL-${yearSuffix}-${Math.floor(1000 + Math.random() * 9000)}`;
                const info = insertClaim.run(
            ...
            } else if (!med.total_amount || med.total_amount === 0) {
                db.prepare('UPDATE claims SET total_amount = ? WHERE id = ?').run(item.medical.total_amount, med.id);
            }
        }
        console.log('[AutoSeed] All 10 dummy test users verified and seeded.');
    } catch (e) {
        console.error('[AutoSeed] Error auto-seeding dummy users:', e.message);
    }
}

module.exports = { autoSeedDummyUsers, defaultTestUsers };
