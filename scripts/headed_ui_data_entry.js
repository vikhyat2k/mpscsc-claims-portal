 ...
 *
 * Exclusively uses the live web frontend UI:
 *   - Launches real Google Chrome with headless: false
 *   - Logs in via https://mpscsc-claims-portal.onrender.com/login
 *   - Creates / verifies Employee Master record
 *   - Creates Tour Diary with multi-leg journeys
 *   - Creates TA/DA Claim with hotel stay and journey legs
 *   - Creates Transfer Claim with packing/transport charges
 *   - Creates Medical Claim with itemized bills
 *   - Repeats for all 10 dummy users
 *   - Tags all records with [DUMMY_DATA_RECORD] for safe future purging
 * ==============================================================================
 */
...
const dummyUserData = [
    {
        ...
        medical: { patient: "Rajesh Sharma", relation: "Self", illness: "Viral Fever & Dehydration [DUMMY_DATA_RECORD]", duration: "3 days", bills: [{ cat: "CONSULTATION", desc: "Dr. Verma Clinic [DUMMY_DATA_RECORD]", amt: "500", no: "RX-101" }, { cat: "MEDICINE", desc: "Apollo Meds [DUMMY_DATA_RECORD]", amt: "1450", no: "AP-201" }] }
    },
    {
        ...
        medical: { patient: "Pooja Verma", relation: "Self", illness: "Eye Strain & Clinic Consultation [DUMMY_DATA_RECORD]", duration: "2 days", bills: [{ cat: "CONSULTATION", desc: "Dr. Agarwal Eye Care [DUMMY_DATA_RECORD]", amt: "600", no: "EYE-88" }, { cat: "MEDICINE", desc: "Eye drops & Vitamins [DUMMY_DATA_RECORD]", amt: "850", no: "RX-902" }] }
    },
    {
        ...
        medical: { patient: "Amitabh Patel", relation: "Self", illness: "Dental Root Canal [DUMMY_DATA_RECORD]", duration: "5 days", bills: [{ cat: "CONSULTATION", desc: "Dental Clinic [DUMMY_DATA_RECORD]", amt: "1200", no: "DEN-44" }, { cat: "MEDICINE", desc: "Pain relief antibiotics [DUMMY_DATA_RECORD]", amt: "720", no: "MED-11" }] }
    },
    {
        ...
        medical: { patient: "Sunita Yadav", relation: "Self", illness: "Hypertension & Checkup [DUMMY_DATA_RECORD]", duration: "3 days", bills: [{ cat: "CONSULTATION", desc: "City Hospital OPD [DUMMY_DATA_RECORD]", amt: "400", no: "OPD-302" }, { cat: "MEDICINE", desc: "Monthly BP Tablets [DUMMY_DATA_RECORD]", amt: "950", no: "PH-788" }] }
    },
    {
        ...
        medical: { patient: "Dinesh Malviya", relation: "Self", illness: "Ankle Sprain [DUMMY_DATA_RECORD]", duration: "4 days", bills: [{ cat: "CONSULTATION", desc: "Orthopedic Clinic [DUMMY_DATA_RECORD]", amt: "500", no: "ORTH-12" }, { cat: "MEDICINE", desc: "Crepe bandage & Ointment [DUMMY_DATA_RECORD]", amt: "640", no: "MED-55" }] }
    },
    {
        ...
        medical: { patient: "Kavita Soni", relation: "Self", illness: "Skin Allergy Treatment [DUMMY_DATA_RECORD]", duration: "3 days", bills: [{ cat: "CONSULTATION", desc: "Skin Care Clinic [DUMMY_DATA_RECORD]", amt: "500", no: "SK-901" }, { cat: "MEDICINE", desc: "Allergy lotion & pills [DUMMY_DATA_RECORD]", amt: "890", no: "MED-23" }] }
    },
    {
        ...
        medical: { patient: "Manoj Tiwari", relation: "Self", illness: "ENT Infection [DUMMY_DATA_RECORD]", duration: "4 days", bills: [{ cat: "CONSULTATION", desc: "ENT Specialist Clinic [DUMMY_DATA_RECORD]", amt: "600", no: "ENT-22" }, { cat: "MEDICINE", desc: "Antibiotics & ear drops [DUMMY_DATA_RECORD]", amt: "810", no: "RX-556" }] }
    },
    {
        ...
        medical: { patient: "Deepak Chouhan", relation: "Self", illness: "Seasonal Bronchitis [DUMMY_DATA_RECORD]", duration: "3 days", bills: [{ cat: "CONSULTATION", desc: "City Hospital OPD [DUMMY_DATA_RECORD]", amt: "350", no: "OPD-89" }, { cat: "MEDICINE", desc: "Cough syrup & tablets [DUMMY_DATA_RECORD]", amt: "520", no: "PH-12" }] }
    },
    {
        ...
        medical: { patient: "Ritu Raghuwanshi", relation: "Self", illness: "Routine Health Checkup & CBC [DUMMY_DATA_RECORD]", duration: "1 day", bills: [{ cat: "TEST", desc: "Pathology CBC Test [DUMMY_DATA_RECORD]", amt: "450", no: "LAB-71" }, { cat: "CONSULTATION", desc: "Doctor Consultation [DUMMY_DATA_RECORD]", amt: "400", no: "DOC-91" }] }
    },
    {
        ...
        medical: { patient: "Sandeep Mishra", relation: "Self", illness: "Seasonal Flu & Fever [DUMMY_DATA_RECORD]", duration: "3 days", bills: [{ cat: "CONSULTATION", desc: "Community Health Center [DUMMY_DATA_RECORD]", amt: "200", no: "CHC-12" }, { cat: "MEDICINE", desc: "Paracetamol & Cough Syrup [DUMMY_DATA_RECORD]", amt: "430", no: "MED-09" }] }
    }
];
...
async function runHeadedDataEntry() {
    ...
    console.log(`  Total Users to Process: ${dummyUserData.length}`);
    ...
    for (let uIdx = 0; uIdx < dummyUserData.length; uIdx++) {
        const item = dummyUserData[uIdx];
        console.log(`\n------------------------------------------------------------------`);
        console.log(`[${uIdx + 1}/${dummyUserData.length}] Processing User: ${item.user.name} (${item.user.email})`);
        console.log(`------------------------------------------------------------------`);
        ...
        // Step 2: Attempt Login Form
        console.log(`  [UI] Entering login credentials for ${item.user.email}...`);
        await page.waitForSelector('input[name="email"]');
        await page.click('input[name="email"]', { clickCount: 3 });
        await page.type('input[name="email"]', item.user.email, { delay: 20 });

        await page.waitForSelector('input[name="password"]');
        await page.click('input[name="password"]', { clickCount: 3 });
        await page.type('input[name="password"]', item.user.pass, { delay: 20 });

        console.log(`  [UI] Clicking Sign In button...`);
        await page.click('button[type="submit"]');

        await page.waitForFunction(() => !!localStorage.getItem('auth_token'), { timeout: 8000 }).catch(() => {});
        await sleep(1000);

        let token = await page.evaluate(() => localStorage.getItem('auth_token'));
        if (!token) {
            console.log(`  [UI] User not found on live instance. Navigating to ${BASE_URL}/register to register via UI...`);
            await page.goto(`${BASE_URL}/register`, { waitUntil: 'networkidle2' });
            await sleep(1000);

            await page.waitForSelector('input[name="full_name"]');
            await page.type('input[name="full_name"]', item.user.name, { delay: 20 });
            await page.type('input[name="email"]', item.user.email, { delay: 20 });
            await page.type('input[name="mobile_number"]', item.user.mobile, { delay: 20 });
            await page.type('input[name="password"]', item.user.pass, { delay: 20 });
            await page.type('input[name="confirm_password"]', item.user.pass, { delay: 20 });
            ...
            await page.waitForFunction(() => !!localStorage.getItem('auth_token'), { timeout: 12000 }).catch(() => {});
            await sleep(1000);

            token = await page.evaluate(() => localStorage.getItem('auth_token'));
            if (!token) {
                console.error(`  ❌ [UI] Critical: Failed to authenticate ${item.user.email} after registration.`);
                continue;
            }
        }
        ...
        const empExists = await page.evaluate((empName) => {
            return document.body.innerText.includes(empName);
        }, item.emp.name);
        ...
        if (!empExists) {
            ...
            const addBtn = await page.evaluateHandle(() => {
                ...
                return btns.find(b => b.innerText.includes('Add Employee') || b.innerText.includes('कर्मचारी जोड़ें'));
            });
            if (addBtn) {
                await addBtn.click();
                await sleep(500);
                ...
                await page.evaluate((empData) => {
                    ...
                    const nameInput = inputs.find(i => i.placeholder && (i.placeholder.includes('Rajesh') || i.placeholder.includes('Name')));
                    if (nameInput) { nameInput.value = empData.name; nameInput.dispatchEvent(new Event('input', { bubbles: true })); }

                    const desigInput = inputs.find(i => i.placeholder && (i.placeholder.includes('District') || i.placeholder.includes('Designation')));
                    if (desigInput) { desigInput.value = empData.desig; desigInput.dispatchEvent(new Event('input', { bubbles: true })); }

                    const hqInput = inputs.find(i => i.placeholder && (i.placeholder.includes('Headquarters') || i.placeholder.includes('Bhopal')));
                    if (hqInput) { hqInput.value = empData.hq; hqInput.dispatchEvent(new Event('input', { bubbles: true })); }

                    const payInput = inputs.find(i => i.placeholder && (i.placeholder.includes('Basic Pay') || i.placeholder.includes('85000')));
                    if (payInput) { payInput.value = empData.pay; payInput.dispatchEvent(new Event('input', { bubbles: true })); }

                    const catSelect = document.querySelector('select');
                    if (catSelect) { catSelect.value = empData.cat; catSelect.dispatchEvent(new Event('change', { bubbles: true })); }
                }, item.emp);
                ...
                await sleep(1200);
                console.log(`  ✅ [UI] Employee record created.`);
            }
        } else {
            console.log(`  ℹ️ [UI] Employee record already exists for ${item.emp.name}.`);
        }
        ...
        // Click "+ Create Tour Diary"
        const createdDiary = await page.evaluate(() => {
            const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('Create Tour Diary') || b.innerText.includes('टूर डायरी बनाएं'));
            if (btn && !btn.disabled) {
                btn.click();
                return true;
            }
        ...
        if (createdDiary) {
            ...
            // Fill journey fields in Tour Diary
            await page.evaluate((d) => {
                const inputs = Array.from(document.querySelectorAll('table input'));
                if (inputs.length >= 6) {
                    const textInputs = inputs.filter(i => i.type === 'text');
                    if (textInputs[0]) { textInputs[0].value = d.from; textInputs[0].dispatchEvent(new Event('input', { bubbles: true })); }
                    if (textInputs[1]) { textInputs[1].value = d.to; textInputs[1].dispatchEvent(new Event('input', { bubbles: true })); }
                    const purposeInput = textInputs.find(i => i.placeholder && i.placeholder.includes('Purpose')) || textInputs[textInputs.length - 1];
                    if (purposeInput) { purposeInput.value = d.purpose; purposeInput.dispatchEvent(new Event('input', { bubbles: true })); }
                }
            }, item.diary);
        ...
        const createdTada = await page.evaluate(() => {
            const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('Create') || b.innerText.includes('दावा'));
            if (btn && !btn.disabled) {
                btn.click();
                return true;
            }
        ...
        const createdTransfer = await page.evaluate(() => {
            const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('Create') || b.innerText.includes('स्थानांतरण'));
            if (btn && !btn.disabled) {
                btn.click();
                return true;
            }
        ...
        if (createdTransfer) {
            ...
            await page.evaluate((tr) => {
                ...
                if (numInputs.length >= 2) {
                    numInputs[0].value = tr.packing;
                    numInputs[0].dispatchEvent(new Event('input', { bubbles: true }));
                    numInputs[1].value = tr.transport;
                    numInputs[1].dispatchEvent(new Event('input', { bubbles: true }));
                }

                const textInputs = Array.from(document.querySelectorAll('input[type="text"]'));
                if (textInputs.length > 0) {
                    textInputs[0].value = tr.family;
                    textInputs[0].dispatchEvent(new Event('input', { bubbles: true }));
                }

                const textareas = Array.from(document.querySelectorAll('textarea'));
                if (textareas.length > 0) {
                    textareas[0].value = tr.remarks;
                    textareas[0].dispatchEvent(new Event('input', { bubbles: true }));
                }
            }, item.transfer);
        ...
        // Select employee from dropdown first
        await page.evaluate(() => {
            const select = document.querySelector('select');
            if (select && select.options.length > 1) {
                select.selectedIndex = 1;
        ...
        const createdMed = await page.evaluate(() => {
            const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('Medical Claim') || b.innerText.includes('चिकित्सा दावा') || b.innerText.includes('Create') || b.innerText.includes('दावा'));
            if (btn && !btn.disabled) {
                btn.click();
                return true;
            }
        ...
        if (createdMed) {
            ...
            await page.evaluate((med) => {
                ...
                // Fill patient, illness, duration
                if (inputs.length >= 2) {
                    inputs[0].value = med.patient;
                    inputs[0].dispatchEvent(new Event('input', { bubbles: true }));
                    inputs[1].value = med.illness;
            ...
            await page.evaluate((med) => {
                ...
                if (numInputs.length > 0) {
                    numInputs[numInputs.length - 1].value = med.bills[0].amt;
                    numInputs[numInputs.length - 1].dispatchEvent(new Event('input', { bubbles: true }));
                }
            }, item.medical);
    ...
    console.log(`\n==================================================================`);
    console.log(`  ALL 10 USERS PROCESSED SUCCESSFULLY VIA HEADED UI AUTOMATION!`);
    console.log(`==================================================================\n`);
