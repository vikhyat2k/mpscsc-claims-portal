...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      try {
        const resClaims = await fetch('http://localhost:5000/api/claims');
  ...
  const handleApprove = (claim) => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          const res = await fetch(`http://localhost:5000/api/claims/${claim.raw_id}/status`, {
            method: 'PUT',
          ...
          showToast(
            ...
            async () => {
              // Undo
              await fetch(`http://localhost:5000/api/claims/${claim.raw_id}/status`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
  ...
  const handleSubmitClaim = (claim) => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          const res = await fetch(`http://localhost:5000/api/claims/${claim.raw_id}/submit`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
